    ...
    <div class="container">
        ...
        <form action="{{ route('pieces.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="marque_piece" id="marque_piece" class="form-control @error('marque_piece') is-invalid @enderror" value="{{ old('marque_piece') }}">
                @error('marque_piece')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="modele_piece" id="modele_piece" class="form-control @error('modele_piece') is-invalid @enderror" value="{{ old('modele_piece') }}">
                @error('modele_piece')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="prix" id="prix" class="form-control @error('prix') is-invalid @enderror" value="{{ old('prix') }}" min="500">
                @error('prix')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="compatible_avec" id="compatible_avec" class="form-control @error('compatible_avec') is-invalid @enderror" value="{{ old('compatible_avec') }}">
            ...
            <div class="form-check mb-3">
                <input type="checkbox" name="disponible" id="disponible" class="form-check-input" {{ old('disponible') ? 'checked' : '' }}>
                <label for="disponible" class="form-check-label">Disponible</label>
            </div>

            <button type="submit" class="btn btn-primary">Ajouter la pièce</button>
        </form>
    </div>
@endsection
